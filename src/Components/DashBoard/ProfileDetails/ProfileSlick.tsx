    ...
    try {
      ...
      if (response.data.Status === 1) {
        const imageObjects = response.data.data.map((img: any) => ({
          id: img.id,
          imageUrl: `http://103.214.132.20:8000${img.image}?t=${new Date().getTime()}` // Add timestamp to avoid caching
        }));
    ...
    if (currentEditIndex !== null) {
      ...
      try {
        ...
        if (imageToRemove.id !== null) {
          ...
          const response = await axios.post('http://103.214.132.20:8000/auth/Remove_profile_img/', formData, {
            headers: {
              'Content-Type': 'multipart/form-data',
            },
          });
