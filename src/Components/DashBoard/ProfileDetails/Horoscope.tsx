...
export const Horoscope = () => {
    ...
    const [selectedBirthStarId, setSelectedBirthStarId] = useState<number | string>('');
    ...
    useEffect(() => {
        const fetchHoroscopeDetails = async () => {
            try {
                const response = await axios.post("http://103.214.132.20:8000/auth/get_myprofile_horoscope/", {
                ...
                const matchedLagnam = lagnams.find(lagnam => lagnam.didi_description.includes(data.personal_lagnam_didi_name));
                if (matchedLagnam) {
    ...
    useEffect(() => {
        const fetchRasis = async () => {
            ...
            try {
                const response = await axios.post('http://103.214.132.20:8000/auth/Get_Rasi/', {
    ...
    const handleBirthStarChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        setFormData(prevState => ({
            ...prevState,
            personal_bthstar_name: event.target.options[event.target.selectedIndex].text
        }));
    };
    ...
    if (
        ...
        !selectedRasiId ||
        !selectedLagnamId ||
        !formData.personal_chevvai_dos ||
        !formData.personal_ragu_dos ||
        !formData.personal_nalikai ||
        !formData.personal_surya_goth ||
        !formData.personal_dasa ||
        !formData.personal_dasa_bal ||
        !formData.personal_rasi_katt ||
        !formData.personal_amsa_katt 
    ) {
        ...
        try {
            const response = await axios.post("http://103.214.132.20:8000/auth/update_myprofile_horoscope/", {
                profile_id: loginuser_profileId,
            ...
            {isEditing ? (
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="space-y-10 my-10">
                        ...
                        <div>
                            <AmsamGridnew data={formData.personal_amsa_katt || ''} onChange={handleAmsamGridChange} centerLabel={"Amsam"}  isEditing={isEditing}  />
                        </div>
                    </div>
                ...
                <div>
                    <div className="grid grid-rows-1 grid-cols-2 gap-4">
                        <div>
                            <h5 className="text-[20px] text-ash font-semibold mb-2">Birth Star:
                                <span className="font-normal"> {horoscopeDetails.personal_bthstar_name}</span>
                            </h5>
                            <h5 className="text-[20px] text-ash font-semibold mb-2">Rasi:
                                <span className="font-normal"> {horoscopeDetails.personal_bth_rasi_name}</span>
                            </h5>
                            <h5 className="text-[20px] text-ash font-semibold mb-2">Lagnam:
                                <span className="font-normal"> {horoscopeDetails.personal_lagnam_didi_name}</span>
                            </h5>
                            <h5 className="text-[20px] text-ash font-semibold mb-2">Dasa Balance:
                                <span className="font-normal"> {horoscopeDetails.personal_dasa_bal}</span>
                            </h5>
                            <h5 className="text-[20px] text-ash font-semibold mb-2">Sarpa Dhosam:
                                <span className="font-normal"> {horoscopeDetails.personal_chevvai_dos}</span>
                            </h5>
                        </div>
                        <div>
                            <h5 className="text-[20px] text-ash font-semibold mb-2">Nallikai:
                                <span className="font-normal"> {horoscopeDetails.personal_nalikai}</span>
                            </h5>
                            <h5 className="text-[20px] text-ash font-semibold mb-2">Suya Gothram:
                                <span className="font-normal"> {horoscopeDetails.personal_surya_goth}</span>
                            </h5>
                            <h5 className="text-[20px] text-ash font-semibold mb-2">Dasa:
                                <span className="font-normal"> {horoscopeDetails.personal_dasa}</span>
                            </h5>
                            <h5 className="text-[20px] text-ash font-semibold mb-2">Chevai Dosham:
                                <span className="font-normal"> {horoscopeDetails.personal_chevvai_dos}</span>
                            </h5>
                        </div>
                    </div>
                    ...
                    <div className="space-y-10 my-10">
                        ...
                        <div>
                            <AmsamGridnew data={horoscopeDetails.personal_amsa_katt || ''} centerLabel={"Amsam"} onChange={handleAmsamGridChange}   isEditing={isEditing}   />
                        </div>
                    </div>
                </div>
            )}
